import { ArrowRight, BatteryCharging, Brush, Car, ClipboardCheck, Headphones, ShieldCheck, UserCheck, Wrench } from "lucide-react";
...
export function CompanyLandingPage() {
  ...
  const kpis = [
    ...
    { label: "AI 自動放行", value: cases.filter((item) => item.assessment.status === "rentable").length, icon: ShieldCheck },
  ...
  return (
    <main className="page company-page">
      <section className="company-hero">
        <div>
          <p className="eyebrow">Company side demo</p>
          <h1>iRent Guard 公司端車況稽核中心</h1>
          <p>整合還車照片、語音備註、車聯網訊號與歷史工單，自動產生可出租判斷、派工優先序與客服摘要。</p>
